...
import { Eye, Pencil, Trash, Bus, Hiking, Search, X } from "lucide-react"; 
...
const Tabla = ({ datos, titulo, eliminarElemento, esActividad }) => {
    ...
    const itemsPorPagina = 10; // Mostrar 10 elementos por página
    ...
    // Calcular total de páginas después del filtrado
    const totalPaginas = Math.ceil(datosFiltrados.length / itemsPorPagina);

    // Obtener elementos de la página actual después del filtrado
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
            {/* Encabezado de la tabla con icono diferente si es actividad o tour */}
            <div className="flex items-center gap-4 mb-4">
                <div className="bg-[#CD9B4A]/10 p-3 rounded-lg">
                    {esActividad ? (
                        <Hiking className="w-6 h-6 text-[#CD9B4A]" />
                    ) : (
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="relative w-full max-w-md">
                    <input
                        ...
                        onChange={(e) => {
                            ...
                            setPaginaActual(1); // Reinicia la paginación en cada búsqueda
                        }}
                ...
                {busqueda && (
                    <button
                        className="ml-2 bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600"
                        onClick={() => {
                            setBusqueda("");
                            setPaginaActual(1);
                        }}
                ...
                <table className="w-full">
                    <thead>
                        <tr className="bg-[#CD9B4A]/10 text-[#CD9B4A] text-sm font-semibold">
                            <th className="px-6 py-4 text-left w-1/12">ID</th>
                            <th className="px-6 py-4 text-left w-3/12">{titulo}</th>
                            <th className="px-6 py-4 text-center w-2/12">Precio</th>
                            <th className="px-6 py-4 text-center w-2/12">Cupo Máximo</th>
                            <th className="px-6 py-4 text-center w-2/12">Duración</th>
                            <th className="px-6 py-4 text-center w-1/12">Detalles</th>
                            <th className="px-6 py-4 text-center w-1/12">Acciones</th>
                        </tr>
                    </thead>
                        ...
                        {datosPaginados.length > 0 ? (
                            datosPaginados.map((item, index) => (
                                <tr key={index} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-center">{item.max_capacity}</td>
                                    <td className="px-6 py-4 text-center">{item.duration} hr</td>
